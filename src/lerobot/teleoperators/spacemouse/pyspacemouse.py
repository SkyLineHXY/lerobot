...
class HidApiDevice:
    """Compatibility wrapper for hidapi devices used when easyhid cannot load libhidapi."""

    def __init__(self, device_info: dict):
        self.path = device_info["path"]
        self.vendor_id = device_info["vendor_id"]
        self.product_id = device_info["product_id"]
        self.release_number = device_info.get("release_number")
        self.manufacturer_string = device_info.get("manufacturer_string")
        self.product_string = device_info.get("product_string")
        self.serial_number = device_info.get("serial_number") or ""
        self._device = None

    def open(self):
        self._device = hidapi.device()
        try:
            path = self.path.encode("utf-8") if isinstance(self.path, str) else self.path
            self._device.open_path(path)
        except OSError as e:
            self._device = None
            raise HIDException("Failed to open device") from e
    ...
    def read(self, size):
        ...
        return bytearray(self._device.read(size))

    def set_nonblocking(self, enable_nonblocking):
        if self._device is None:
            raise HIDException("HIDDevice not open")
        self._device.set_nonblocking(enable_nonblocking)


class HidApiEnumeration:
    """Subset of easyhid.Enumeration backed by the hidapi Python package."""

    def __init__(self, vid=0, pid=0):
        if hidapi is None:
            raise RuntimeError("The hidapi Python package is not installed.")
        self.device_list = [HidApiDevice(device) for device in hidapi.enumerate(vid, pid)]

    def find(self):
        return self.device_list


def _new_hid_enumeration():
    try:
        return EasyHIDEnumeration()
    except AttributeError:
        return HidApiEnumeration()
...
def list_devices():
    ...
    devices = []
    try:
        hid = _new_hid_enumeration()
    except Exception as e:
        raise Exception(
            "HID API is unavailable. Install a working hidapi backend (for example the 'hidapi' Python package "
            "or the system libhidapi library), then retry."
        ) from e
